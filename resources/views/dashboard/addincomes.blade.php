            ...
            html +=`
              <tr class="${color}">
                    ...
                    if(row.status_code=="001"){
                      html+=`<td><button class="btn btn-sm btn-primary btnsave" data-id="${row.id}" data-amount="${row.item.item_amount}" data-deliveryfee="${Number(row.item.delivery_fees)+Number(row.item.other_fees)}" data-deposit="${row.item.deposit}" data-paystatus="${row.item.paystatus}">save</button></td>`
